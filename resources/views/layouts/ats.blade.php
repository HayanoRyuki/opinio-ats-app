...
<body>
@if (! $isLoginPage)
<nav style="height:56px;background:#332c54;color:#f4f4ed;display:flex;align-items:center;">
  ...
  <ul style="display:flex;gap:32px;list-style:none;margin:0;padding-left:24px;">
    @foreach ([
      'candidates'  => ['👥','候補者'],
      'jobs'        => ['💼','求人'],
      'evaluations' => ['⭐','評価'],
  ...
  <div style="margin-left:auto;padding-right:24px;display:flex;gap:16px;">
    <a href="/notifications" style="{{ $linkDone }}">🔔</a>
    <a href="/me" style="{{ $linkDone }}">👤</a>
    @if ($isAdminLike)
      <a href="/settings" style="{{ $linkDone }}">⚙️</a>
    @endif
  </div>
</nav>
@endif
...
@if (! $isLoginPage)
<nav style="width:240px;background:#332c54;color:#f4f4ed;font-size:14px;line-height:1.6;height:calc(100vh - 56px);">
